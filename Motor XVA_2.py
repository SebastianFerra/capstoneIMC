...
######### hay que hacerle a esta info el cambio de formato visto en el excel (o buscar curva)
#####################################
####################
####################
#################
...
## Debería ser input?? # los primeros 2 sacarlos de un poco más arriba
# fixings historicos para derivados anteriores 
...
########
# years=[2,5,10,20] # esto valoriza todos los días de año +2//+5//+10//+20
# plazos sacar algunos plazos es decir valorizar 2 veces por año o recibir input o variable nput
years = [1]
results = []
results_EFV = {}
...
for y in years:
    ...
    for x in range(1,DIAS_POR_ANO+1):
        year = "2022"
        res = dt.strptime(year + "-" + str(x), "%Y-%j").strftime("%m-%d-%Y")
        print("res", res)
        date = [int(year), int(res[:2]), int(res[3:5])]
        results.append(valorize_mtm(date,ex))
print(results)
