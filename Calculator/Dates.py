...
from Calculator import dateutils
...
def _yearfrac(dt1, dt2, convention, **kwargs):
...
def _yearfractions(dates, convention, **kwargs):
    ...
    return map(lambda x: _yearfrac(dates[0], x, convention, **kwargs), dts)


def _daydiff(dt1, dt2, convention, **kwargs):
...
def get_yf(start_date, end_date, convention):
    base = '365' if '365' in convention else '360'
    daycount = 'act' if 'act' in convention.lower() else '30'

    conv = daycount + '/' + base if daycount == 'act' else '30/360 us'
    yf = _yearfrac(start_date, end_date, conv)
